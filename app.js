...
// Professional profiles database - Now with numbered IDs (1-20)
const profiles = {
    ...
    // QR Detection handler - Core functionality - UPDATED FOR NUMBERED IDs
    function handleQRDetection(qrData) {
        try {
            // Clean and normalize the QR data to extract numeric profile ID
            let profileId = qrData
                .trim()
                .replace(/^https?:\/\/[^\/]+\//, "") // Remove URL prefix
                .replace(/^.*profile=/, "") // Remove profile parameter
                .replace(/^.*id=/, "") // Remove id parameter
            ...
            // Check if profile ID is valid (1-20) and not already displayed
            if (profileId >= 1 && profileId <= 20 && profiles[profileId] && !currentProfiles.has(profileId)) {
            ...
            } else if (!profileId || profileId < 1 || profileId > 20 || !profiles[profileId]) {
